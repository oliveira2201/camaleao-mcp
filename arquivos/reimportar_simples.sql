...
SELECT
    ...
    id::text,
    (key->>'fromMe')::boolean,
    COALESCE("pushName", CASE WHEN (key->>'fromMe')::boolean THEN 'Camaleao' ELSE 'Cliente' END)::text,
    CASE
    ...
    CASE
        ...
        ELSE NULL
    END::text,
    TO_TIMESTAMP("messageTimestamp")::timestamptz,
    jsonb_build_object(
        'id', id,
        'key', key,
        'pushName', "pushName",
...
SELECT COUNT(*) as total, COUNT(CASE WHEN raw_payload IS NOT NULL AND raw_payload != '{}'::jsonb THEN 1 END) as com_payload, COUNT(CASE WHEN media_url IS NOT NULL THEN 1 END) as com_media_url FROM wa_mensagens WHERE instancia = 'camaleao';
