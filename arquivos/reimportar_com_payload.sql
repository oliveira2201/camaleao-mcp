-- ============================================
-- RE-IMPORTAÇÃO COM raw_payload COMPLETO
-- ============================================
-- Este script importa mensagens do banco Evolution
-- para o banco principal com raw_payload completo
-- ============================================

-- Criar extensão dblink se não existir
CREATE EXTENSION IF NOT EXISTS dblink;

-- Importar mensagens (TESTE - 100 mensagens)
...
SELECT
    ...
    COUNT(*) as mensagens_inseridas
FROM wa_mensagens
WHERE instancia = 'camaleao';

-- Verificar se raw_payload foi salvo
SELECT
    'VERIFICAÇÃO raw_payload' as status,
    COUNT(*) as total,
    COUNT(CASE WHEN raw_payload IS NOT NULL AND raw_payload != '{}'::jsonb THEN 1 END) as com_payload,
